...
// ==========================================
// DAFTAR DOMAIN ANDA
// Tambahkan atau hapus domain di dalam array ini
// ==========================================
const domainList = [
    'vlez.eu.cc',
    'vlzxr.eu.cc',
...
bot.action('check_join', async (ctx) => {
    ...
    if (isJoined) {
        ...
        const keyboard = Markup.inlineKeyboard([
            [Markup.button.callback('🚀 Create Account VLESS', 'select_domain')]
        ]);
...
bot.action('select_domain', async (ctx) => {
    ...
    domainList.forEach((domain, index) => {
        buttons.push([Markup.button.callback(`🌐 ${domain}`, `setdomain_${index}`)]);
    });
...
async function showCountryMenu(ctx) {
    ...
    buttons.push([
        Markup.button.callback('« Back', 'select_domain'), 
        Markup.button.callback('❌ Cancel', 'back_home')
    ]);
...
// Handler saat ISP/Provider dipilih
for (const countryCode of Object.keys(dataServer)) {
    dataServer[countryCode].providers.forEach(prov => {
        bot.action(`prov_${prov.id}`, async (ctx) => {
            if (!userSession[ctx.from.id]) return ctx.reply('Sesi habis, silakan ketik /start ulang.');
...
bot.action('mode_domain', async (ctx) => {
    const session = userSession[ctx.from.id];
    if (!session || !session.domain) return ctx.reply('Sesi habis, silakan ketik /start ulang.');
...
// ==========================================
// GENERATE CONFIG VLESS
// ==========================================
async function generateAndSendVless(ctx) {
    const session = userSession[ctx.from.id];
    const countryData = dataServer[session.country];
    const prov = session.provider;
    const selectedDomain = session.domain; // Menggunakan domain yang dipilih user
...
\`${configLink}\`
    ...
    const keyboard = Markup.inlineKeyboard([
        [Markup.button.callback('🔄 Buat Lagi', 'select_domain')]
    ]);
